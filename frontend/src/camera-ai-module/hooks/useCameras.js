...
const useCameras = () => {
  ...
  const loadCamerasProgressively = async () => {
    try {
      const url = `${CENTRAL_URL}/api/camera-ai/cameras`;
      console.log("[Cameras] Loading from URL:", url);
  ...
  useEffect(() => {
    ...
    //Su dung CENTRAL_URL de tao WebSocket URL
  ...
  const fetchCameras = async () => {
    ...
    try {
      const response = await fetch(`${CENTRAL_URL}/api/camera-ai/cameras`);
      const data = await response.json();
      if (data.success) {
